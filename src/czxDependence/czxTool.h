...
public:
	...
	static bool saveMatrix4f(vector<Matrix4f> matrixs, int num, string file_name)
	{
		ofstream o;
		o.open(file_name);
		for (int i = 0; i < num; i++)
	...
	static vector<Matrix4f> loadMatrix4f(int num, string filename)
	{
		vector<Matrix4f> ret(num);
		//ret.reserve(num);
		ifstream in;
		in.open(filename);
		for (int i = 0; i < num; i++)
