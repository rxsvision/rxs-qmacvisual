...
#pragma region 图像处理
		...
		//光度立体法
		if (nPhotometricSteroState_buf == 1)
		{
			nPhotometricSteroState = getPhotometricSteroState();
			if (nPhotometricSteroState < 10 && nPhotometricSteroState>0)
			{
				dataVar::int_link = nPhotometricSteroState;
				frmLink* fLink = new frmLink();
				fLink->exec();
				setPhotometricSteroState();
				nPhotometricSteroState = 0;
			}
		}
		//图像细化
...
#pragma region 三维检测
		...
		//平面度测量
		if (nFlatness3DState_buf == 1)
		{
			nFlatness3DState = getFlatness3DState();
			if (nFlatness3DState > 0)
			{
				dataVar::int_link = nFlatness3DState;
...
ToolNameList frmProItemTab::GetProcessItemNum(const QString itemName)
{
	...
	if (itemName.contains("光度立体法")) return ToolNameList::PHOTOMETRICSTERO;
	if (itemName.contains("多焦距合成")) return ToolNameList::FOCUSFUSION;
	if (itemName.contains("HDR")) return ToolNameList::HDR;
	...
	if (itemName.contains("单圆标定")) return ToolNameList::SINGLECIRCLE_CALIBRATION;
	...
	if (itemName.contains("寻找SLOT")) return ToolNameList::FIND_SLOT;
	...
	if (itemName.contains("平面度计算")) return ToolNameList::FLATNESS3D;
...
#pragma region 图像处理
	...
	case PHOTOMETRICSTERO: {
		//光度立体法
		QLibrary mylib("./Plugins/photometricStero.dll");   //声明所用到的dll文件
		if (mylib.load())    //判断是否正确加载
		{
			getPhotometricSteroState = (GetUniform)mylib.resolve("ShowFormState");
			setPhotometricSteroState = (SetUniform)mylib.resolve("SetFormState");
			Funs open = (Funs)mylib.resolve("showDialog");
			if (open)
			{
				nPhotometricSteroState_buf = 1;
				Toolnterface* frmPage = open(sToolName, QConfig::ToolBase[flow_index]);
				frmPage->setObjectName(sToolName);
				return frmPage;
			}
	...
	case FOCUSFUSION: {
		//多焦距合成
		QLibrary mylib("./Plugins/FocusFusion_D.dll");   //声明所用到的dll文件
		if (mylib.load())    //判断是否正确加载
		{
			getPerspectiveTransformState = (GetPerspectiveTransform)mylib.resolve("ShowFormState");
			setPerspectiveTransformState = (SetPerspectiveTransform)mylib.resolve("SetFormState");
	...
	case HDR: {
		//HDR
		QLibrary mylib("./Plugins/HDR.dll");   //声明所用到的dll文件
		if (mylib.load())    //判断是否正确加载
		{
			getPerspectiveTransformState = (GetPerspectiveTransform)mylib.resolve("ShowFormState");
			setPerspectiveTransformState = (SetPerspectiveTransform)mylib.resolve("SetFormState");
...
#pragma region 标定工具
	...
	case SINGLECIRCLE_CALIBRATION: {
		//测量标定
		QLibrary mylib("./Plugins/SingleCircleCalibration.dll");   //声明所用到的dll文件
		if (mylib.load())    //判断是否正确加载
		{
			getMeasureCalibrationState = (GetMeasureCalibration)mylib.resolve("ShowFormState");
			setMeasureCalibrationState = (SetMeasureCalibration)mylib.resolve("SetFormState");
...
#pragma region 几何工具
	...
	case FIND_SLOT: {
		//寻找直线
		QLibrary mylib("./Plugins/FindSlot.dll");   //声明所用到的dll文件
		if (mylib.load())    //判断是否正确加载
		{
			getFindLineState = (GetFindLine)mylib.resolve("ShowFormState");
			setFindLineState = (SetFindLine)mylib.resolve("SetFormState");
...
#pragma region 三维检测
	...
	case FLATNESS3D: {
		//平面度测量
		QLibrary mylib("./Plugins/Flatness3D.dll");   //声明所用到的dll文件
		if (mylib.load())    //判断是否正确加载
		{
			getFlatness3DState = (GetUniform)mylib.resolve("ShowFormState");
			setFlatness3DState = (SetUniform)mylib.resolve("SetFormState");
			Funs open = (Funs)mylib.resolve("showDialog");
			if (open)
			{
				nFlatness3DState_buf = 1;
				Toolnterface* frmPage = open(sToolName, QConfig::ToolBase[flow_index]);
				frmPage->setObjectName(sToolName);
				return frmPage;
			}
