	...
	for (int k = 0; k < parent.size() && k < widgetInfoLists_save.size(); k++)
	{
		...
		for (int i = 0; i < widgetInfoLists.size(); i++) {
			for (int j = 0; j < listQLineEdit.size(); j++)
			{
				...
				QString a = le->objectName();
				QString b = widgetInfoLists.at(i).control_name;
				if (le->objectName() == widgetInfoLists.at(i).control_name)
				{
...
void QtWidgetsSaveLoad::saveToolParamCZX(int index, QString tool_name, QVariant var)
{
	dataOther[index].append(var);
}
...
void  QtWidgetsSaveLoad::saveToFile(QList<QWidget*> parent)
{
	...
	if (file.open(QIODevice::ReadWrite))
	{
		...
		stream << widgetInfoLists_save << dataInfoList << dataToolList << dataToolTitleList << dataIoLocalList
			...
			<< dataSocketTcpClientList << dataSocketTcpServerList<<dataOther;
		file.close();
	}
	else
	{
	...
	for (int m = 0; m < 20; m++)
	{
		if (QConfig::ToolBase[m] != nullptr)
		{
			...
			for (int n = 0; n < QConfig::ToolBase[m]->m_Tools.size(); n++)
			{
				...
				{
					++other_count;
					QVariant var = dataOther[m].at(other_count);
