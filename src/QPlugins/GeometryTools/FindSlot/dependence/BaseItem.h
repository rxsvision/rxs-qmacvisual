...
//四个控制点，内圈左上点，内圈右上点，内圈下线任意点，外圈左上点
class SlotItem : public BaseItem
{
	Q_OBJECT
public:
	SlotItem(qreal x, qreal y, qreal width, qreal height, qreal thickness=30);
	SlotItem(QPointF ilt, QPointF irt, QPointF id, QPointF olt);
	...
	inline QVector<QPointF> getControlPoints() {
...
private:
	...
	QVector<QPointF> eight_points;//双层slot的8个角点
};
...
public:	
	...
	int FindBorderPoints(const cv::Mat& src_mat, std::vector<QPointF>& xy);
