...
frmHDR::frmHDR(QString toolName, QToolBase* toolBase, QWidget* parent)
	: Toolnterface(toolName, toolBase, parent)
{
	...
	connect(this, &frmHDR::clickTab, this, &frmHDR::onTabClicked);
...
int frmHDR::ExecuteLink(const int int_link, const QString str_link, const QMap<QString, gVariable::Global_Var> variable_link)
{
	try
	{
...
int frmHDR::InitSetToolData(const QVariant data)
{
	try
	{
		frmHDRData init_data;
		init_data = data.value<frmHDRData>();
		...
		for (int i = 0; i < init_data.img_links.size(); i++)
		{
			...
			ip->ui.time->setObjectName(QString("time") + QString::number(i));
			connect(ip, &FocusFusionParameter::linkClicked, this, &frmHDR::on_subBtnLinkImage_clicked);
...
QVariant frmHDR::InitGetToolData()
{
	frmHDRData data;
...
int frmHDR::Execute(const QString toolname)
{
	for (int i = 0; i < GetToolBase()->m_Tools.size(); i++)
	{
...
int frmHDR::RunToolPro()
{
	try
	{
		...
		for (int i = 0; i < ui.img_parameters->count() - 1; i++)
		{
			...
			times.push_back(ip->ui.time->text().toFloat());
...
bool frmHDR::eventFilter(QObject* obj, QEvent* event)
{
	if (obj == ui.img_parameters->tabBar() && event->type() == QEvent::MouseButtonPress) {
		...
		else if (mouseEvent->button() == Qt::RightButton)
		{
			...
			//// 处理菜单项的点击事件
			connect(action1, &QAction::triggered, this, &frmHDR::onTabClose);
...
void frmHDR::onTabClicked(int index)
{
	if (index >= 0)
	{
		...
		if (tab_name == "+")
		{
			...
			if (new_index >= 0) {  // 确保索引有效
				...
				ip->ui.time->setObjectName(QString("time") + QString::number(new_index));
...
void frmHDR::on_btnAddROI_clicked()
{
	view->ClearObj();
	rectangle_item->deleteLater();
	rectangle_item = new RectangleItem(10, 10, 200, 200);
...
void frmHDR::on_btnLinkFollow_clicked()
{
	QConfig::nFormState = 3;
}

void frmHDR::on_btnLinkImage_clicked()
{
	QConfig::nFormState = 1;
}

void frmHDR::on_btnExecute_clicked()
{
...
void frmHDR::on_subBtnLinkImage_clicked()
{
	QConfig::nFormState = 2;
}

void frmHDR::on_btnDeleteROI_clicked()
{
